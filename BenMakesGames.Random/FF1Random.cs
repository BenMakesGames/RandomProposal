using System;

namespace BenMakesGames.Random;

/// <summary>
/// `FF1Random` loops through the same sequence of bytes over and over again. The exact values used are taken from the
/// NES game Final Fantasy 1, which used these values to determine when "random" enemy encounters should occur.
///
/// Example use: creating a more speed-runner friendly game by incorporating intentionally-exploitable RNG.
///
/// Distinguishing properties:
/// * Uses a lookup table to generate random bytes
/// </summary>
public sealed class FF1Random: IRandom
{
    public static IRandom Shared => new FF1Random();

    private int Index;

    public void FillBytes(Span<byte> bytes)
    {
        if (Index + bytes.Length <= ByteTable.Length)
        {
            // copy the bytes in one go
            ByteTable.AsSpan().Slice(Index, bytes.Length).CopyTo(bytes);
            
            Index = (Index + bytes.Length) % ByteTable.Length;

            return;
        }

        while(bytes.Length > 0)
        {
            int smallestBytesAvailable = Math.Min(ByteTable.Length - Index, bytes.Length);
            
            ByteTable.AsSpan().Slice(Index, smallestBytesAvailable).CopyTo(bytes);
            bytes = bytes[smallestBytesAvailable..];
            
            Index = (Index + smallestBytesAvailable) % ByteTable.Length;
        }
    }

    public FF1Random(int seed = 0)
    {
        Index = seed % 256; // using ByteTable.Length here results in an exception
    }

    // from https://tasvideos.org/GameResources/NES/FinalFantasy1#EncounterAlgorithms
    private static readonly byte[] ByteTable =
    {
        
    };
}
